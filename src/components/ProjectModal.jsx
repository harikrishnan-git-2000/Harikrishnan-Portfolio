...
const ProjectModal = ({ project, onClose }) => {
    ...
    const renderDetailContent = (detail, index) => {
        ...
        switch (detail.type.toLowerCase()) {
            ...
            case 'buttonlink': return (<div key={index} className="my-4"> <a href={detail.href} target="_blank" rel="noopener noreferrer" className="px-5 py-2 rounded-lg bg-transparent border border-white-50 text-white-50 hover:bg-white-50 hover:text-black transition-colors duration-300">{detail.label || 'Link'}</a></div>);
